...
const CreateServerPopup: React.FC<CreateServerPopupProps> = ({ onClose, userId }) => {
    ...
    useEffect(() => {
        const fetchServers = async () => {
            try {
                ...
                const userServers = data.filter((server: Server) => server.createdByUserId === userId);
            ...
            <div className="relative bg-white p-6 rounded-lg shadow-lg w-full max-w-xl">
                    ...
                    <div className="">
                        ...
                        <div className="mt-2 cursor-pointer">
                            {servers.map((server) => (
                                <div
                                    key={server._id}
                                    className={`bg-gray-100 w-full rounded-md py-4 items-center mb-4 flex ${server.channelName === 'SpecialChannel' ? 'bg-blue-200' : 'bg-gray-100'
                                        }`}
                                >
                                    <Image src={ImageServer} alt="Img" className="float-left ml-5" />
                                    <div className="font-bold ml-5">
                ...
                {step === 4 && (
                    <div>
                        ...
                        <button onClick={onClose} className="mt-4 w-full py-2 bg-[#68A86B] text-white rounded">
                            Close
                        </button>
                    </div>
                )}
