...
const CreateServerPopup: React.FC<CombinedProps> = ({ onClose, userId, onServerSelect }) => {
    ...
    const [serverImageUpload, setServerImageUpload] = useState<string | null>(null);
    ...
    const handleCreate = async (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (!nameRegex.test(serverName)) {
            toast.error('Server name must contain letters and may include numbers, and must be between 2 to 15 characters');
            return;
        }
                ...
                <div className="relative bg-white p-6 rounded-lg shadow-lg w-full max-w-xl">
                    ...
                    {step === 1 && (
                        <div className="">
                            ...
                            <div onClick={() => { setServerType('custom'); handleNext(); }} className='cursor-pointer'>
                                <div className="bg-gray-100 w-full rounded-md py-4 items-center flex">
                                    <Image src={ImageServer} alt="Img" className="float-left ml-5" />
                            ...
                            {servers.map((server) => (
                                <div
                                    ...
                                    onClick={() => onServerSelect && onServerSelect({ serverId: server._id, serverName: server.channelName })}
                                >
                                    <div
                                        className={`w-full rounded-md py-4 items-center mb-4 flex ${server.channelName === 'SpecialChannel' ? 'bg-blue-200' : 'bg-gray-100'}`}
                                    >
                                        <Image src={ImageServer} alt="Img" className="float-left ml-5" />
                                        <div className="font-bold ml-5">
                                            <h2 className={`text-xl font-semibold ${server.channelName === 'SpecialChannel' ? 'text-blue-700' : 'text-black'}`}>
                                                {server.channelName}
                                            </h2>
                                        </div>
                                        <Image src={ImageNext} alt="Img" className="ml-auto mr-6" />
                    ...
                    {step === 2 && (
                        <div>
                            ...
                            <div onClick={() => { setChannelType('friends'); handleNext(); }} className='cursor-pointer'>
                                <div className="bg-gray-100 w-full rounded-md py-4 items-center flex">
                                    <Image src={ImageServer} alt="Img" className="float-left ml-5" />
                                    <div className="font-bold ml-5">
                            ...
                            <div onClick={() => { setChannelType('community'); handleNext(); }} className='mt-4 cursor-pointer'>
                                <div className="bg-gray-100 w-full rounded-md py-4 items-center flex">
                                    <Image src={ImageServer} alt="Img" className="float-left ml-5" />
                                    <div className="font-bold ml-5">
                                        For my team
                    ...
                    {step === 3 && (
                        <div>
                            ...
                            <div className="w-full rounded-md py-8 cursor-pointer" onClick={handleImageClick}>
                                {serverImageUpload ? (
                                    <Image src={serverImageUpload} alt="Uploaded Preview" className="bg-center bg-no-repeat w-32 h-32 object-cover rounded-md mx-auto" width={32} height={32} />
                                ) : (
                                    <Image src={uploadImage} alt="Upload" className="bg-center bg-no-repeat w-32 h-32 object-cover rounded-md mx-auto" />
                                )}
                            </div>
                            ...
                            <input
                                ...
                                accept="image/*"
                            />
                            <label className="block text-black font-semibold">Server Name</label>
                            <input
                                type="text"
                                value={serverName}
                    ...
                    {step === 4 && (
                        <div>
                            <h2 className="text-xl font-bold text-center">Server Created Successfully!</h2>
                            <div className='flex justify-center'>
                                <button onClick={handleClose} className="mt-4 w-[20%] py-2 bg-[#68A86B] text-white rounded hover:bg-green-100 hover:text-black border border-[#68A86B]">
                                    Close
                                </button>
                            </div>
                        </div>
